...
function piplapi_date_parse_from_format($format, $date)
{
...
function piplapi_str_to_datetime($s)
{
...
function piplapi_datetime_to_str($dt)
{
    ...
    return $dt->format(PIPLAPI_TIMESTAMP_FORMAT);
}

function piplapi_str_to_date($s)
{
    // Transform an string to a DateTime object.
    $parsed = piplapi_date_parse_from_format(PIPLAPI_DATE_FORMAT, $s);
    validate_datetime($parsed);
    return new DateTime(sprintf('%04d-%02d-%02d %02d:%02d:%02d',
                                            $parsed['year'],
                                            $parsed['month'],
